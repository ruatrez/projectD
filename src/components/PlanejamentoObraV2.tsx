...
import {Plus, Eye, Edit, Trash2, Calendar, DollarSign, Building, Users, Package, AlertTriangle, Clock, CheckCircle, XCircle, Filter, Search, ChevronLeft, ChevronRight, FileText, MessageSquare, Target, Activity, Save, X, ArrowRight, Download, PlusCircle, Link, ArrowUp, ArrowDown, Calculator, Wallet, ShoppingCart, UserPlus, Send, AlertCircle, RefreshCw, Settings, BarChart3, TrendingUp, Bell} from 'lucide-react'
...
export default function PlanejamentoObraV2() {
  ...
  const fetchData = async () => {
    try {
      ...
      // Trazer TODOS os contratos cadastrados, independente do status
  ...
  const deletePlanejamento = async () => {
    if (!currentPlanejamento) {
      toast.error('Selecione uma obra para excluir')
      return
    }

    const confirmacao = window.confirm(
      `Tem certeza que deseja excluir o planejamento da obra "${currentPlanejamento.nomeProjeto}"?\n\nEsta ação não pode ser desfeita e todos os dados relacionados (etapas, subetapas, materiais, mão de obra e solicitações) serão permanentemente removidos.`
    )

    if (!confirmacao) return

    try {
      await lumi.entities.planejamentos.delete(currentPlanejamento._id)
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex justify-between items-center">
        <div>
          <h1 className="text-2xl font-bold text-gray-900">Planejamento de Obra V2</h1>
          ...
          <div className="flex items-center space-x-4 flex-1">
            ...
            <select
              ...
              onChange={async (e) => {
                ...
                // Verificar se já existe planejamento vinculado a este contrato
                const planejamentoExistente = planejamentos.find(p => p.contratoId === contratoId)
                
                if (planejamentoExistente) {
                  // Já existe planejamento, apenas seleciona
                  setSelectedPlanejamento(planejamentoExistente._id)
                } else {
                  // Não existe planejamento, criar automaticamente
                  toast.loading('Criando planejamento automaticamente...', { id: 'auto-planejamento' })
                  await createPlanejamentoFromContrato(contratoId)
                  toast.success('Planejamento criado! Adicione etapas para continuar.', { id: 'auto-planejamento' })
                }
              }}
            ...
            >
              ...
              {contratos.map(contrato => {
                const temPlanejamento = planejamentos.some(p => p.contratoId === contrato._id)
                const statusLabel = {
                ...
                return (
                  <option key={contrato._id} value={contrato._id}>
                    {contrato.nomeProjeto} - {contrato.cliente} | {statusLabel} | {formatCurrency(contrato.valorTotal)} {temPlanejamento ? '✓ Planejado' : '→ Criar Planejamento'}
                  </option>
                )
              })}
            </select>
